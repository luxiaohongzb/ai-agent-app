...
public class AiAgentChatService implements IAiAgentChatService {
    ...
    @Override
    public String aiAgentChat(String aiAgentId, String message) {
        log.info("智能体对话请求，参数 {} {}", aiAgentId, message);

        List<String> aiClientIds = repository.queryAiClientIdsByAiAgentId(aiAgentId);

        String content = "";

        for (String aiClientId : aiClientIds) {
    ...
    public Flux<ChatResponse> aiAgentChatStream(String aiAgentId, String ragId, String message) {
        ...
        if (StringUtils.isNoneBlank(ragId)){
            // 查询RAG标签
